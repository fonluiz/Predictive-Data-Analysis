...
```
Para responder essa questão, é interessante vermos as distribuição das médias de cada curso em um boxplot para sabermos onde está a mediana e o primeiro e o terceiro quartis das notas de cada curso. 
...
Um indicativo que encontrei para tentar saber porque esse cursos possuem melhores e piores notas foi o percentual de alunos reprovados por faltas dentre os alunos de cada curso.
```{r}
...
ggplot(reprovacoes_por_falta, aes(y = Percent_Reprovados_Por_Falta, x = reorder(Nome_Curso_Campus, Percent_Reprovados_Por_Falta), fill = as.factor(Campus))) +
  ...
  coord_flip()
```
Os cursos de Física e Estatística do campus 1, que possuem as piores notas, também possuem os índices mais altos de reprovados por falta. E os cursos de Medicina e Direito (M e V) possuem os menores índices de reprovações por falta assim como possuem as notas mais altas. É claro que essa não é uma regra, já que há cursos dentre os que tem notas mais altas que não estão dentre os que possuem menores indices de reprovados por falta. Mas sabendo que os alunos reprovados por falta são contabilizados como nota 0, é de se esperar que a mediana de um curso com altos índices de reprovações por falta sofra influência dessas notas baixas.
Desconsiderando os alunos repro'
