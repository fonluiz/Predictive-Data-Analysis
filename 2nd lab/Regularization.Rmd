...
students.train.data <- students.train.data %>% 
  filter(!is.na(media))
students.test.data <- students.test.data %>% 
  filter(!is.na(media))

# Calcula o CRA de cada aluno
alunos.cra.treino <- students.train.data %>%
  group_by(matricula) %>%
  mutate(cra.contrb = media*creditos) %>%
  summarise(cra = round(sum(cra.contrb)/sum(creditos), 2))

alunos.cra.test <- students.test.data %>%
  group_by(matricula) %>%
  mutate(cra.contrb = media*creditos) %>%
  summarise(cra = round(sum(cra.contrb)/sum(creditos), 2))
...
O melhor modelo gerado, levando em consideração o modelo que apresentou menor RMSE, foi o modelo cujo lambda é 0.1629751. O RMSE foi igual a 0.5666946 e o R² foi igual a 0.5831063 para o treinamento. É importante saber que esses valores podem mudar ligeiramente a cada vez que rodamos o treino.
...
ridge.rmse <- get.rmse(ridge.pred - students.test.data$cra).
df <- data.frame(pred=ridge.pred, obs=students.test.data$cra)
round(defaultSummary(df), digits = 3)

lasso.pred <- predict(lasso.fit, students.test.data)
lasso.rmse <- get.rmse(lasso.pred - students.test.data$cra)
