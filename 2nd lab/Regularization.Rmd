...
pcr.pred <- predict(pcr.fit, students.test.data, ncomp=7)
pcr.rmse <- rmse(pcr.pred - students.test.data$cra)
pcr.rmse
...
cra_parcial <- function(dataframe) {
  dataframe$cra.parcial <- round(((
  rowMeans(
  dataframe %>% select(
  Álgebra.Vetorial.e.Geometria.Analítica,
  Cálculo.Diferencial.e.Integral.II,
  ...
  dataframe$Teoria.dos.Grafos * 2
  )) / 6, 2)
  
  return(dataframe)
}

students.train.data <- cra_parcial(students.train.data)
students.train.data <- cra_parcial(students.test.data)


fit = lm(cra ~ poly(cra.parcial, degree = 4), data=students.train.data)
summary(fit)
pred <- predict(fit, students.test.data)
```

```{r}
teste.kaggle <- read.csv("data/test.csv")
kaggle.pred <- predict(pcr.fit, teste.kaggle, ncomp=7)
